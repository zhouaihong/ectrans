...
INTEGER(KIND=JPIM) :: JM,JN,ILENGTH,ISTATUS
REAL(KIND=JPRD) :: ZR(KM,KN)
REAL(KIND=JPRD),ALLOCATABLE :: ZS(:,:),ZT(:,:)
CHARACTER(LEN=16) :: ZQR_VALUE
LOGICAL :: LUSE_LAPACK_QR
!----------------------------------------------------------------------------
!Avoid destroying input matrix
ZR(:,:) = PMAT(1:KM,1:KN)
CALL GET_ENVIRONMENT_VARIABLE('ECTRANS_ID_LAPACK_QR',ZQR_VALUE,&
 & LENGTH=ILENGTH,STATUS=ISTATUS)
LUSE_LAPACK_QR=.TRUE.
IF (ISTATUS == 0 .AND. ILENGTH > 0) LUSE_LAPACK_QR=ZQR_VALUE(1:1) /= '0'
IF (LUSE_LAPACK_QR) THEN
  CALL LAPACK_QRCP(PEPS,KM,KN,ZR,KRANK,KBCLIST)
ELSE
...
!==============================================================================
SUBROUTINE LAPACK_QRCP(PEPS,KM,KN,PA,KRANK,KLIST)
IMPLICIT NONE

REAL(KIND=JPRD),INTENT(IN)     :: PEPS
INTEGER(KIND=JPIM),INTENT(IN)  :: KM,KN
REAL(KIND=JPRD),INTENT(INOUT)  :: PA(:,:)
INTEGER(KIND=JPIM),INTENT(OUT) :: KRANK
INTEGER(KIND=JPIM),INTENT(OUT) :: KLIST(:)

INTEGER(KIND=JPIM) :: IMIN,INFO,ISVD_RANK,LWORK
INTEGER(KIND=JPIM),ALLOCATABLE :: IJPVT(:)
REAL(KIND=JPRD) :: ZR11
REAL(KIND=JPRD),ALLOCATABLE :: ZTAU(:),ZWORK(:)
CHARACTER(LEN=16) :: ZSVD_VALUE
INTEGER :: ILENGTH,ISTATUS
LOGICAL :: LUSE_SVD_RANK

IMIN=MIN(KM,KN)
CALL GET_ENVIRONMENT_VARIABLE('ECTRANS_ID_SVD_RANK',ZSVD_VALUE,&
 & LENGTH=ILENGTH,STATUS=ISTATUS)
LUSE_SVD_RANK=ISTATUS == 0 .AND. ILENGTH > 0 .AND. ZSVD_VALUE(1:1) == '1'
IF (LUSE_SVD_RANK) CALL SVD_NUMERICAL_RANK(PEPS,KM,KN,PA,ISVD_RANK)
ALLOCATE(IJPVT(KN),ZTAU(IMIN),ZWORK(1))
IJPVT=0
LWORK=-1
CALL DGEQP3(KM,KN,PA,KM,IJPVT,ZTAU,ZWORK,LWORK,INFO)
IF (INFO /= 0) CALL ABOR1('DGEQP3 workspace query failed in LAPACK_QRCP')
LWORK=MAX(1,INT(ZWORK(1),KIND=JPIM))
DEALLOCATE(ZWORK)
ALLOCATE(ZWORK(LWORK))
IJPVT=0
CALL DGEQP3(KM,KN,PA,KM,IJPVT,ZTAU,ZWORK,LWORK,INFO)
IF (INFO /= 0) CALL ABOR1('DGEQP3 failed in LAPACK_QRCP')

KLIST(1:KN)=IJPVT
KRANK=0
ZR11=ABS(PA(1,1))
DO WHILE (KRANK < IMIN)
  IF (ABS(PA(KRANK+1,KRANK+1)) <= PEPS*ZR11) EXIT
  KRANK=KRANK+1
ENDDO
IF (LUSE_SVD_RANK) KRANK=MAX(1,MIN(IMIN,ISVD_RANK))
DEALLOCATE(IJPVT,ZTAU,ZWORK)
END SUBROUTINE LAPACK_QRCP

!==============================================================================
SUBROUTINE SVD_NUMERICAL_RANK(PEPS,KM,KN,PA,KRANK)
IMPLICIT NONE

REAL(KIND=JPRD),INTENT(IN) :: PEPS,PA(:,:)
INTEGER(KIND=JPIM),INTENT(IN) :: KM,KN
INTEGER(KIND=JPIM),INTENT(OUT) :: KRANK

INTEGER(KIND=JPIM) :: IMIN,INFO,LWORK
REAL(KIND=JPRD) :: ZU(1,1),ZVT(1,1)
REAL(KIND=JPRD),ALLOCATABLE :: ZA(:,:),ZS(:),ZWORK(:)

IMIN=MIN(KM,KN)
ALLOCATE(ZA(KM,KN),ZS(IMIN),ZWORK(1))
ZA=PA(1:KM,1:KN)
LWORK=-1
CALL DGESVD('N','N',KM,KN,ZA,KM,ZS,ZU,1,ZVT,1,ZWORK,LWORK,INFO)
IF (INFO /= 0) CALL ABOR1('DGESVD workspace query failed in SVD_NUMERICAL_RANK')
LWORK=MAX(1,INT(ZWORK(1),KIND=JPIM))
DEALLOCATE(ZWORK)
ALLOCATE(ZWORK(LWORK))
ZA=PA(1:KM,1:KN)
CALL DGESVD('N','N',KM,KN,ZA,KM,ZS,ZU,1,ZVT,1,ZWORK,LWORK,INFO)
IF (INFO /= 0) CALL ABOR1('DGESVD failed in SVD_NUMERICAL_RANK')

KRANK=0
DO WHILE (KRANK < IMIN)
  IF (ZS(KRANK+1) <= PEPS*ZS(1)) EXIT
  KRANK=KRANK+1
ENDDO
DEALLOCATE(ZA,ZS,ZWORK)
END SUBROUTINE SVD_NUMERICAL_RANK
...
ZTAU_REC= ZTAU
KRANK = 0
DO WHILE (ZTAU > PEPS**2*ZTAU_IN .AND. KRANK < IMIN)
  KRANK = KRANK+1
  IF( KRANK <= IMIN ) THEN
    ILIST(KRANK) = IK
