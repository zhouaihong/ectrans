...
PUBLIC NODE_TYPE,LEV_STRUCT,BUTTERFLY_STRUCT,CONSTRUCT_BUTTERFLY,MULT_BUTV,MULT_BUTM,CLONE,&
 ...
 & BUTTERFLY_COST,ANALYZE_TRANSPOSED_HYBRID
...
TYPE(BUTTERFLY_STRUCT) :: YTRANSPOSE
...
DO IB=1,YHYBRID%BLOCK_COUNT
  ...
  KBLOCKS=KBLOCKS+1
  ICONTRIB(B%FIRST_COL:B%FIRST_COL+B%NCOLS-1)=&
    & ICONTRIB(B%FIRST_COL:B%FIRST_COL+B%NCOLS-1)+1
  ALLOCATE(ZTRANSPOSE(B%NCOLS,B%NROWS))
  ZTRANSPOSE=TRANSPOSE(PMAT(B%FIRST_ROW:B%FIRST_ROW+B%NROWS-1,&
    & B%FIRST_COL:B%FIRST_COL+B%NCOLS-1))
  ICMAX=MIN(KCMAX,B%NROWS)
  IF (B%KIND == HYBRID_LOW_RANK) ICMAX=B%NROWS
  CALL CONSTRUCT_BUTTERFLY(PEPS,ICMAX,B%NCOLS,B%NROWS,&
    & ZTRANSPOSE,YTRANSPOSE)
  ICOST=BUTTERFLY_COST(YTRANSPOSE)
  IF (ICOST < B%DENSE_COST) THEN
    KFACTOR=KFACTOR+ICOST
    KSELECTED=KSELECTED+1
  ELSE
    KFACTOR=KFACTOR+B%DENSE_COST
  ENDIF
  CALL FREE_BUTTERFLY(YTRANSPOSE)
