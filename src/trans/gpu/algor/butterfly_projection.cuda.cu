...
constexpr int wmma_k_tile = 8;
constexpr int warp_threads = 32;
constexpr int wmma_field_tile = 128;
...
__global__ void butterfly_grouped_gemm_kernel(
  ...
  for (int field_base = first_field;
    ...
    for (int column_base = first_column;
      ...
      for (int k_base = 0; k_base < k; k_base += wmma_k_tile) {
        ...
        for (int index = thread; index < wmma_k_tile * gemm_tile;
             index += blockDim.x) {
          const int b_k = index % wmma_k_tile;
        ...
        for (int k_tile = 0;
             k_tile < wmma_k_tile && k_base + k_tile < k; k_tile += 4) {
    ...
    for (int rank_base = 0; rank_base < rank; rank_base += gemm_tile) {
      ...
      for (int k_base = 0; k_base < nonidentity; k_base += wmma_k_tile) {
        ...
        for (int index = thread; index < wmma_k_tile * gemm_tile;
             index += blockDim.x) {
          const int local_k = index % wmma_k_tile;
          const int local_rank = index / wmma_k_tile;
    ...
    for (int row_base = 0; row_base < row_count; row_base += gemm_tile) {
      ...
      for (int k_base = 0; k_base < rank; k_base += wmma_k_tile) {
        ...
        for (int index = thread; index < wmma_k_tile * gemm_tile;
             index += blockDim.x) {
          const int local_k = index % wmma_k_tile;
          const int local_row = index / wmma_k_tile;
