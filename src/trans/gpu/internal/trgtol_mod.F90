...
MODULE TRGTOL_MOD
  ...
  LOGICAL, SAVE :: LTRGTOL_MAP_CACHE_READY = .FALSE.
  LOGICAL, SAVE :: LTRGTOL_MAP_CACHE_ON_DEVICE = .FALSE.
  LOGICAL, SAVE :: C_LLOCAL_CONTRIBUTION = .FALSE.

  INTEGER(KIND=JPIM), SAVE :: C_KF_GP = -1
  INTEGER(KIND=JPIM), SAVE :: C_KF_FS = -1
  INTEGER(KIND=JPIM), SAVE :: C_NCUR_RESOL = -1
  INTEGER(KIND=JPIM), SAVE :: C_NPROC = -1
  INTEGER(KIND=JPIM), SAVE :: C_NPRTRV = -1
  INTEGER(KIND=JPIM), SAVE :: C_NPRTRW = -1
  INTEGER(KIND=JPIM), SAVE :: C_MYPROC = -1
  INTEGER(KIND=JPIM), SAVE :: C_MYSETV = -1
  INTEGER(KIND=JPIM), SAVE :: C_MYSETW = -1
  INTEGER(KIND=JPIM), SAVE :: C_MY_REGION_NS = -1
  INTEGER(KIND=JPIM), SAVE :: C_MY_REGION_EW = -1
  INTEGER(KIND=JPIM), SAVE :: C_NLENGTF = -1

  INTEGER(KIND=JPIM), ALLOCATABLE, SAVE :: C_IVSET(:)
  INTEGER(KIND=JPIM), ALLOCATABLE, SAVE :: C_PGP_INDICES(:)
  INTEGER(KIND=JPIM), ALLOCATABLE, SAVE :: C_ISEND_FIELD_COUNT(:)
  INTEGER(KIND=JPIM), ALLOCATABLE, SAVE :: C_ISEND_WSET_SIZE(:)
  INTEGER(KIND=JPIM), ALLOCATABLE, SAVE :: C_ISEND_WSET_OFFSET(:)
  INTEGER(KIND=JPIB), ALLOCATABLE, SAVE :: C_IRECV_BUFR_TO_OUT(:,:)
  INTEGER(KIND=JPIB), ALLOCATABLE, SAVE :: C_IRECV_BUFR_TO_OUT_OFFSET(:)
  INTEGER(KIND=JPIB), ALLOCATABLE, SAVE :: C_ISENDTOT(:)
  INTEGER(KIND=JPIB), ALLOCATABLE, SAVE :: C_IRECVTOT(:)
...
#ifdef ACCGPU
    IF (LTRGTOL_MAP_CACHE_ON_DEVICE) THEN
      !$ACC EXIT DATA DELETE(C_IRECV_BUFR_TO_OUT,C_PGP_INDICES)
    ENDIF
#endif
    IF (ALLOCATED(C_IVSET)) DEALLOCATE(C_IVSET)
    IF (ALLOCATED(C_PGP_INDICES)) DEALLOCATE(C_PGP_INDICES)
    IF (ALLOCATED(C_ISEND_FIELD_COUNT)) DEALLOCATE(C_ISEND_FIELD_COUNT)
    IF (ALLOCATED(C_ISEND_WSET_SIZE)) DEALLOCATE(C_ISEND_WSET_SIZE)
    IF (ALLOCATED(C_ISEND_WSET_OFFSET)) DEALLOCATE(C_ISEND_WSET_OFFSET)
    IF (ALLOCATED(C_IRECV_BUFR_TO_OUT)) DEALLOCATE(C_IRECV_BUFR_TO_OUT)
    IF (ALLOCATED(C_IRECV_BUFR_TO_OUT_OFFSET)) DEALLOCATE(C_IRECV_BUFR_TO_OUT_OFFSET)
    IF (ALLOCATED(C_ISENDTOT)) DEALLOCATE(C_ISENDTOT)
    IF (ALLOCATED(C_IRECVTOT)) DEALLOCATE(C_IRECVTOT)
    LTRGTOL_MAP_CACHE_READY = .FALSE.
    LTRGTOL_MAP_CACHE_ON_DEVICE = .FALSE.
...
#define TRGTOL_DTYPE MPI_REAL8
    ...
    LCACHE_VALID = LTRGTOL_MAP_CACHE_READY
    IF (LCACHE_VALID) LCACHE_VALID = C_KF_GP == KF_GP .AND. C_KF_FS == KF_FS
    IF (LCACHE_VALID) LCACHE_VALID = C_NCUR_RESOL == NCUR_RESOL .AND. C_NPROC == NPROC
    IF (LCACHE_VALID) LCACHE_VALID = C_NPRTRV == NPRTRV .AND. C_NPRTRW == NPRTRW
    IF (LCACHE_VALID) LCACHE_VALID = C_MYPROC == MYPROC .AND. C_MYSETV == MYSETV .AND. C_MYSETW == MYSETW
    IF (LCACHE_VALID) LCACHE_VALID = C_MY_REGION_NS == MY_REGION_NS .AND. C_MY_REGION_EW == MY_REGION_EW
    IF (LCACHE_VALID) LCACHE_VALID = C_NLENGTF == D%NLENGTF
    IF (LCACHE_VALID) LCACHE_VALID = ALLOCATED(C_IVSET)
    IF (LCACHE_VALID) LCACHE_VALID = SIZE(C_IVSET) == KF_GP
    IF (LCACHE_VALID) LCACHE_VALID = ALL(C_IVSET == IVSET)
    IF (LCACHE_VALID) LCACHE_VALID = ALLOCATED(C_PGP_INDICES)
    IF (LCACHE_VALID) LCACHE_VALID = SIZE(C_PGP_INDICES) == PGP_INDICES_END
    IF (LCACHE_VALID) LCACHE_VALID = ALL(C_PGP_INDICES == PGP_INDICES)

    IF (.NOT. LCACHE_VALID) THEN
      CALL RESET_TRGTOL_CACHE()

      ALLOCATE(C_IVSET(KF_GP))
      ALLOCATE(C_PGP_INDICES(PGP_INDICES_END))
      ALLOCATE(C_ISEND_FIELD_COUNT(NPRTRV))
      ALLOCATE(C_ISEND_WSET_SIZE(NPRTRW))
      ALLOCATE(C_ISEND_WSET_OFFSET(NPRTRW+1))
      ALLOCATE(C_IRECV_BUFR_TO_OUT(D%NLENGTF,2))
      ALLOCATE(C_IRECV_BUFR_TO_OUT_OFFSET(NPROC))
      ALLOCATE(C_ISENDTOT(NPROC))
      ALLOCATE(C_IRECVTOT(NPROC))
      ...
      IF(NPRTRV == 1) THEN
        ...
        C_ISEND_FIELD_COUNT(1) = KF_GP
      ELSE
        C_ISEND_FIELD_COUNT(:) = 0
        DO JFLD=1,KF_GP
          C_ISEND_FIELD_COUNT(IVSET(JFLD)) = C_ISEND_FIELD_COUNT(IVSET(JFLD)) + 1
        ENDDO
      ENDIF
      ...
      DO ILOCAL_LAT=D%NFRSTLAT(MY_REGION_NS),D%NLSTLAT(MY_REGION_NS)
        ...
        C_ISEND_WSET_SIZE(D%NPROCL(ILOCAL_LAT)) = &
            & C_ISEND_WSET_SIZE(D%NPROCL(ILOCAL_LAT))+D%NONL(ILAT_STRIP,MY_REGION_EW)
      ENDDO
      ! sum up offsets
      C_ISEND_WSET_OFFSET(1) = 0
      DO JROC=1,NPRTRW
        C_ISEND_WSET_OFFSET(JROC+1)=C_ISEND_WSET_OFFSET(JROC)+C_ISEND_WSET_SIZE(JROC)
      ENDDO
      DO JROC=1,NPROC
        CALL PE2SET(JROC,ISETA,ISETB,ISETW,ISETV)
        ! total send size is # points per field * # fields
        C_ISENDTOT(JROC) = 1_JPIB*C_ISEND_WSET_SIZE(ISETW)*C_ISEND_FIELD_COUNT(ISETV)
      ENDDO
      C_LLOCAL_CONTRIBUTION = C_ISENDTOT(MYPROC) > 0
      CALL GSTATS(481,1)
      CALL GSTATS(1820,1)
      ...
      CALL GSTATS(1821,0)
      CALL GSTATS(482,0)
      C_IRECV_BUFR_TO_OUT_OFFSET(:) = 0
      DO JROC=1,NPROC
        ! Get new offset to my current KINDEX entry
        IF (JROC > 1 .AND. KF_FS > 0) THEN
          C_IRECV_BUFR_TO_OUT_OFFSET(JROC) = C_IRECV_BUFR_TO_OUT_OFFSET(JROC-1)+C_IRECVTOT(JROC-1)/KF_FS
        ELSEIF (JROC > 1) THEN
          C_IRECV_BUFR_TO_OUT_OFFSET(JROC) = C_IRECV_BUFR_TO_OUT_OFFSET(JROC-1)
        ENDIF
        ...
        DO JGL=IFIRSTLAT,ILASTLAT
          ...
          DO JL=1,D%NONL(IGL,ISETB)
            ...
            C_IRECV_BUFR_TO_OUT(C_IRECV_BUFR_TO_OUT_OFFSET(JROC)+IPOS,1) = &
                & KF_FS*D%NSTAGTF(IGLL)+(D%NSTA(IGL,ISETB)-1)+(JL-1)
            ! distance between two layers of this gridpoint
            C_IRECV_BUFR_TO_OUT(C_IRECV_BUFR_TO_OUT_OFFSET(JROC)+IPOS,2) = &
                & D%NSTAGTF(IGLL+1)-D%NSTAGTF(IGLL)
          ENDDO
        ENDDO
        !we always receive the full fourier space
        C_IRECVTOT(JROC) = IPOS*KF_FS
      ENDDO
      CALL GSTATS(482,1)
      CALL GSTATS(1821,1)

      C_IVSET(:) = IVSET(:)
      C_PGP_INDICES(:) = PGP_INDICES(:)
      C_KF_GP = KF_GP
      C_KF_FS = KF_FS
      C_NCUR_RESOL = NCUR_RESOL
      C_NPROC = NPROC
      C_NPRTRV = NPRTRV
      C_NPRTRW = NPRTRW
      C_MYPROC = MYPROC
      C_MYSETV = MYSETV
      C_MYSETW = MYSETW
      C_MY_REGION_NS = MY_REGION_NS
      C_MY_REGION_EW = MY_REGION_EW
      C_NLENGTF = D%NLENGTF
#ifdef ACCGPU
      !$ACC ENTER DATA COPYIN(C_IRECV_BUFR_TO_OUT,C_PGP_INDICES) ASYNC(1)
      LTRGTOL_MAP_CACHE_ON_DEVICE = .TRUE.
#endif
      LTRGTOL_MAP_CACHE_READY = .TRUE.
    ENDIF

    ISEND_FIELD_COUNT(:) = C_ISEND_FIELD_COUNT(:)
    ISEND_WSET_SIZE(:) = C_ISEND_WSET_SIZE(:)
    ISEND_WSET_OFFSET(:) = C_ISEND_WSET_OFFSET(:)
    IRECV_BUFR_TO_OUT_OFFSET(:) = C_IRECV_BUFR_TO_OUT_OFFSET(:)
    ISENDTOT(:) = C_ISENDTOT(:)
    IRECVTOT(:) = C_IRECVTOT(:)
    LLOCAL_CONTRIBUTION = C_LLOCAL_CONTRIBUTION
