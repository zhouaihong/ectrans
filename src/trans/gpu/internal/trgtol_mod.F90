...
MODULE TRGTOL_MOD
  ...
  PRIVATE
  PUBLIC :: TRGTOL_HANDLE, TRGTOL, PREPARE_TRGTOL, SET_TRGTOL_UPDATE_DEVICE, RESET_TRGTOL_CACHE
  ...
  TYPE(TRGTOL_MAP_PLAN), POINTER, SAVE :: TRGTOL_MAP_PLANS => NULL()
...
CONTAINS
  ...
  SUBROUTINE TRGTOL(ALLOCATOR,HTRGTOL,PREEL_REAL,KF_FS,KF_GP,KF_UV_G,KF_SCALARS_G,&
    ...
    USE TPM_GEN,                ONLY: LSYNC_TRANS, LMPOFF, NCUR_RESOL
...
#define TRGTOL_DTYPE MPI_REAL8
    ...
    !$OMP CRITICAL(TRGTOL_PLAN_CACHE_MANAGER)
    CACHE => TRGTOL_MAP_PLANS
    DO WHILE (ASSOCIATED(CACHE))
      LCACHE_VALID = CACHE%KF_GP == KF_GP .AND. CACHE%KF_FS == KF_FS
...
#endif
      CACHE%NEXT => TRGTOL_MAP_PLANS
      TRGTOL_MAP_PLANS => CACHE
    ENDIF
    !$OMP END CRITICAL(TRGTOL_PLAN_CACHE_MANAGER)
