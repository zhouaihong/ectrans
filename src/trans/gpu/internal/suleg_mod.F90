...
#ifndef TRANS_SINGLE
USE BUTTERFLY_ALG_DP_MOD,ONLY: CONSTRUCT_BUTTERFLY
  ...
  IF (BUTTERFLY_REQUESTED()) THEN
    DO IMLOC=1,D%NUMP
      ...
      IF (ILA > ITHRESHOLD) THEN
        ...
        CALL CONSTRUCT_BUTTERFLY(ZEPS_INT_DEC,IMAXCOLS,IDGLU,ILA,&
          & S%FA(IMLOC)%RPNMDA,S%FA(IMLOC)%YBUT_STRUCT_A)
      ...
      IF (ILS > ITHRESHOLD) THEN
        ...
        CALL CONSTRUCT_BUTTERFLY(ZEPS_INT_DEC,IMAXCOLS,IDGLU,ILS,&
          & S%FA(IMLOC)%RPNMDS,S%FA(IMLOC)%YBUT_STRUCT_S)
