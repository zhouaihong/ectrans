...
USE BUTTERFLY_ALG_DP_MOD,ONLY: CONSTRUCT_HYBRID,ANALYZE_LEGENDRE_TRUNCATION,&
  & LEGENDRE_TRUNCATION_DIAGNOSTICS_REQUESTED,CONSTRUCT_BUTTERFLY,&
  & FREE_BUTTERFLY,BUTTERFLY_COST,BUTTERFLY_STRUCT,&
  & ANALYZE_TRANSPOSED_HYBRID,CONSTRUCT_TRANSPOSED_HYBRID
...
INTEGER(KIND=JPIM) :: INM, IM, IRECV, ISEND, ISREQ, IRREQ, &
             ...
             &INX, ISL, ISTART, ITHRESHOLD, INSMAX, IMAXCOLS,IHYBRID_LEAF,IHYBRID_MINCOLS,&
             &IHYBRID_LEAF_DEFAULT,ILATSMAX,JW,JV,J, &
...
INTEGER(KIND=JPIB) :: IDIRECT_DENSE(2),IDIRECT_FACTOR(2),IDIRECT_MATRIX_DENSE,&
  & IDIRECT_MATRIX_FACTOR
INTEGER(KIND=JPIB) :: IDIRECT_HYBRID_DENSE(2),IDIRECT_HYBRID_FACTOR(2),&
  & IDIRECT_HYBRID_CONTRIB(2),IDIRECT_MATRIX_HYBRID_DENSE,&
  & IDIRECT_MATRIX_HYBRID_FACTOR,IDIRECT_MATRIX_CONTRIB
...
REAL(KIND=JPRD) :: ZTRUNC_COST_RATIO,ZTRUNC_ERROR,ZDIRECT_COST_RATIO
REAL(KIND=JPRD),ALLOCATABLE :: ZDIRECT_TRANSPOSE(:,:)
TYPE(BUTTERFLY_STRUCT) :: YDIRECT_TRANSPOSE
INTEGER(KIND=JPIM) :: IDIRECT_MATRICES(2),IDIRECT_MAX_LEVELS(2)
INTEGER(KIND=JPIM) :: IDIRECT_HYBRID_BLOCKS(2),IDIRECT_HYBRID_SELECTED(2),&
  & IDIRECT_HYBRID_OUTPUTS(2),IDIRECT_HYBRID_MAX_CONTRIB(2),&
  & IDIRECT_MATRIX_BLOCKS,IDIRECT_MATRIX_SELECTED,IDIRECT_MATRIX_MAX_CONTRIB
LOGICAL :: LTRUNC_DIAGNOSTICS,LDIRECT_TRANSPOSE_DIAGNOSTICS
...
IHYBRID_LEAF_DEFAULT=IHYBRID_LEAF
LBUTTERFLY_MAXCOLS_SET=.FALSE.
#ifndef TRANS_SINGLE
LDIRECT_TRANSPOSE_DIAGNOSTICS=.FALSE.
CALL GET_ENVIRONMENT_VARIABLE('ECTRANS_GPU_DIRECT_TRANSPOSE_DIAGNOSTICS',&
  & CBUTTERFLY_MAXCOLS,LENGTH=IBUTTERFLY_ENV_LENGTH,&
  & STATUS=IBUTTERFLY_ENV_STATUS)
IF (IBUTTERFLY_ENV_STATUS == 0 .AND. IBUTTERFLY_ENV_LENGTH > 0) &
  & LDIRECT_TRANSPOSE_DIAGNOSTICS=&
  & TRIM(CBUTTERFLY_MAXCOLS(1:IBUTTERFLY_ENV_LENGTH)) /= '0'
...
ENDIF
CALL GET_ENVIRONMENT_VARIABLE('ECTRANS_GPU_HYBRID_LEAF',&
  & CBUTTERFLY_MAXCOLS,LENGTH=IBUTTERFLY_ENV_LENGTH,&
  & STATUS=IBUTTERFLY_ENV_STATUS)
IF (IBUTTERFLY_ENV_STATUS == 0 .AND. IBUTTERFLY_ENV_LENGTH > 0) THEN
  READ(CBUTTERFLY_MAXCOLS(1:IBUTTERFLY_ENV_LENGTH),*,&
    & IOSTAT=IBUTTERFLY_ENV_STATUS) IHYBRID_LEAF
  IF (IBUTTERFLY_ENV_STATUS /= 0 .OR. IHYBRID_LEAF <= 0) &
    & IHYBRID_LEAF=IHYBRID_LEAF_DEFAULT
ENDIF
#endif
ITHRESHOLD = MAX(ITHRESHOLD,IHYBRID_LEAF+1,IMAXCOLS+1)
#ifndef TRANS_SINGLE
IHYBRID_MINCOLS=ITHRESHOLD
CALL GET_ENVIRONMENT_VARIABLE('ECTRANS_GPU_HYBRID_MINCOLS',&
  & CBUTTERFLY_MAXCOLS,LENGTH=IBUTTERFLY_ENV_LENGTH,&
  & STATUS=IBUTTERFLY_ENV_STATUS)
IF (IBUTTERFLY_ENV_STATUS == 0 .AND. IBUTTERFLY_ENV_LENGTH > 0) THEN
  READ(CBUTTERFLY_MAXCOLS(1:IBUTTERFLY_ENV_LENGTH),*,&
    & IOSTAT=IBUTTERFLY_ENV_STATUS) IHYBRID_MINCOLS
  IF (IBUTTERFLY_ENV_STATUS /= 0 .OR. IHYBRID_MINCOLS < 0) &
    & IHYBRID_MINCOLS=ITHRESHOLD
ENDIF
#endif
S%ITHRESHOLD = ITHRESHOLD
#ifndef TRANS_SINGLE
IF (BUTTERFLY_REQUESTED()) WRITE(NOUT,'(A,I0,A,I0,A,I0,A,I0)') &
  & 'GPU butterfly maxcols=',IMAXCOLS,' hybrid_leaf=',IHYBRID_LEAF,&
  & ' threshold=',ITHRESHOLD,' hybrid_mincols=',IHYBRID_MINCOLS
#endif
...
! not correct logic
  ...
  IF (LDIRECT_TRANSPOSE_DIAGNOSTICS) THEN
    IDIRECT_DENSE=0
    IDIRECT_FACTOR=0
    IDIRECT_MATRICES=0
    IDIRECT_MAX_LEVELS=0
    DO IMLOC=1,D%NUMP
      IM=D%MYMS(IMLOC)
      IF (IM == 0) CYCLE
      ILA=(R%NSMAX-IM+2)/2
      ILS=(R%NSMAX-IM+3)/2
      IDGLU=MIN(R%NDGNH,G%NDGLU(IM))
      IF (ILA > ITHRESHOLD .AND. IDGLU > 0) THEN
        ALLOCATE(ZDIRECT_TRANSPOSE(ILA,IDGLU))
        ZDIRECT_TRANSPOSE=TRANSPOSE(REAL(S%FA(IMLOC)%RPNMA(:,:),JPRD))
        CALL CONSTRUCT_BUTTERFLY(ZEPS_INT_DEC,MIN(IMAXCOLS,IDGLU),&
          & ILA,IDGLU,ZDIRECT_TRANSPOSE,YDIRECT_TRANSPOSE)
        IDIRECT_MATRIX_DENSE=INT(ILA,JPIB)*INT(IDGLU,JPIB)
        IDIRECT_MATRIX_FACTOR=BUTTERFLY_COST(YDIRECT_TRANSPOSE)
        IDIRECT_DENSE(1)=IDIRECT_DENSE(1)+IDIRECT_MATRIX_DENSE
        IDIRECT_FACTOR(1)=IDIRECT_FACTOR(1)+IDIRECT_MATRIX_FACTOR
        IDIRECT_MATRICES(1)=IDIRECT_MATRICES(1)+1
        IDIRECT_MAX_LEVELS(1)=MAX(IDIRECT_MAX_LEVELS(1),&
          & YDIRECT_TRANSPOSE%N_LEVELS)
        ZDIRECT_COST_RATIO=REAL(IDIRECT_MATRIX_FACTOR,JPRD)/&
          & REAL(IDIRECT_MATRIX_DENSE,JPRD)
        WRITE(NOUT,'(A,A,A,I0,A,I0,A,I0,A,I0,A,I0,A,I0,A,F8.5)') &
          & 'GPU_DIRECT_TRANSPOSE_MATRIX path=','A',' m=',IM,&
          & ' rows=',ILA,' cols=',IDGLU,' levels=',YDIRECT_TRANSPOSE%N_LEVELS,&
          & ' dense_cost=',IDIRECT_MATRIX_DENSE,&
          & ' factor_cost=',IDIRECT_MATRIX_FACTOR,&
          & ' cost_ratio=',ZDIRECT_COST_RATIO
        CALL FREE_BUTTERFLY(YDIRECT_TRANSPOSE)
        DEALLOCATE(ZDIRECT_TRANSPOSE)
      ENDIF
      IF (ILS > ITHRESHOLD .AND. IDGLU > 0) THEN
        ALLOCATE(ZDIRECT_TRANSPOSE(ILS,IDGLU))
        ZDIRECT_TRANSPOSE=TRANSPOSE(REAL(S%FA(IMLOC)%RPNMS(:,:),JPRD))
        CALL CONSTRUCT_BUTTERFLY(ZEPS_INT_DEC,MIN(IMAXCOLS,IDGLU),&
          & ILS,IDGLU,ZDIRECT_TRANSPOSE,YDIRECT_TRANSPOSE)
        IDIRECT_MATRIX_DENSE=INT(ILS,JPIB)*INT(IDGLU,JPIB)
        IDIRECT_MATRIX_FACTOR=BUTTERFLY_COST(YDIRECT_TRANSPOSE)
        IDIRECT_DENSE(2)=IDIRECT_DENSE(2)+IDIRECT_MATRIX_DENSE
        IDIRECT_FACTOR(2)=IDIRECT_FACTOR(2)+IDIRECT_MATRIX_FACTOR
        IDIRECT_MATRICES(2)=IDIRECT_MATRICES(2)+1
        IDIRECT_MAX_LEVELS(2)=MAX(IDIRECT_MAX_LEVELS(2),&
          & YDIRECT_TRANSPOSE%N_LEVELS)
        ZDIRECT_COST_RATIO=REAL(IDIRECT_MATRIX_FACTOR,JPRD)/&
          & REAL(IDIRECT_MATRIX_DENSE,JPRD)
        WRITE(NOUT,'(A,A,A,I0,A,I0,A,I0,A,I0,A,I0,A,I0,A,F8.5)') &
          & 'GPU_DIRECT_TRANSPOSE_MATRIX path=','S',' m=',IM,&
          & ' rows=',ILS,' cols=',IDGLU,' levels=',YDIRECT_TRANSPOSE%N_LEVELS,&
          & ' dense_cost=',IDIRECT_MATRIX_DENSE,&
          & ' factor_cost=',IDIRECT_MATRIX_FACTOR,&
          & ' cost_ratio=',ZDIRECT_COST_RATIO
        CALL FREE_BUTTERFLY(YDIRECT_TRANSPOSE)
    ...
    DO IPATH=1,2
      ...
      ELSE
        CTRUNC_PATH='S'
      ENDIF
      ZDIRECT_COST_RATIO=REAL(IDIRECT_FACTOR(IPATH),JPRD)/&
        & REAL(IDIRECT_DENSE(IPATH),JPRD)
      WRITE(NOUT,'(A,A,A,I0,A,I0,A,I0,A,I0,A,F8.5)') &
        & 'GPU_DIRECT_TRANSPOSE path=',CTRUNC_PATH,&
        & ' matrices=',IDIRECT_MATRICES(IPATH),&
        & ' max_levels=',IDIRECT_MAX_LEVELS(IPATH),&
        & ' dense_cost=',IDIRECT_DENSE(IPATH),&
        & ' factor_cost=',IDIRECT_FACTOR(IPATH),&
        & ' cost_ratio=',ZDIRECT_COST_RATIO
    ENDDO
  ENDIF

  IF (BUTTERFLY_REQUESTED()) THEN
    IDIRECT_HYBRID_DENSE=0
    IDIRECT_HYBRID_FACTOR=0
    IDIRECT_HYBRID_CONTRIB=0
    IDIRECT_HYBRID_BLOCKS=0
    IDIRECT_HYBRID_SELECTED=0
    IDIRECT_HYBRID_OUTPUTS=0
    IDIRECT_HYBRID_MAX_CONTRIB=0
    DO IMLOC=1,D%NUMP
      IM = D%MYMS(IMLOC)
      IF (IM == 0) CYCLE
      ISL = MAX(R%NDGNH-G%NDGLU(IM)+1,1)
      ...
      IF (ILA > IHYBRID_MINCOLS) THEN
        ...
        IF (LDIRECT_TRANSPOSE_DIAGNOSTICS) THEN
          CALL ANALYZE_TRANSPOSED_HYBRID(ZEPS_INT_DEC,IMAXCOLS,&
            & S%FA(IMLOC)%RPNMDA,S%FA(IMLOC)%YHYBRID_A,&
            & IDIRECT_MATRIX_HYBRID_DENSE,IDIRECT_MATRIX_HYBRID_FACTOR,&
            & IDIRECT_MATRIX_BLOCKS,IDIRECT_MATRIX_SELECTED,&
            & IDIRECT_MATRIX_CONTRIB,IDIRECT_MATRIX_MAX_CONTRIB)
          IDIRECT_HYBRID_DENSE(1)=IDIRECT_HYBRID_DENSE(1)+&
            & IDIRECT_MATRIX_HYBRID_DENSE
          IDIRECT_HYBRID_FACTOR(1)=IDIRECT_HYBRID_FACTOR(1)+&
            & IDIRECT_MATRIX_HYBRID_FACTOR
          IDIRECT_HYBRID_BLOCKS(1)=IDIRECT_HYBRID_BLOCKS(1)+&
            & IDIRECT_MATRIX_BLOCKS
          IDIRECT_HYBRID_SELECTED(1)=IDIRECT_HYBRID_SELECTED(1)+&
            & IDIRECT_MATRIX_SELECTED
          IDIRECT_HYBRID_CONTRIB(1)=IDIRECT_HYBRID_CONTRIB(1)+&
            & IDIRECT_MATRIX_CONTRIB
          IDIRECT_HYBRID_OUTPUTS(1)=IDIRECT_HYBRID_OUTPUTS(1)+ILA
          IDIRECT_HYBRID_MAX_CONTRIB(1)=MAX(IDIRECT_HYBRID_MAX_CONTRIB(1),&
            & IDIRECT_MATRIX_MAX_CONTRIB)
          WRITE(NOUT,'(A,A,A,I0,A,I0,A,I0,A,I0,A,I0,A,F8.5,A,F8.3,A,I0)') &
            & 'GPU_DIRECT_TRANSPOSE_HYBRID_MATRIX path=','A',' m=',IM,&
            & ' blocks=',IDIRECT_MATRIX_BLOCKS,&
            & ' selected=',IDIRECT_MATRIX_SELECTED,&
            & ' dense_cost=',IDIRECT_MATRIX_HYBRID_DENSE,&
            & ' factor_cost=',IDIRECT_MATRIX_HYBRID_FACTOR,&
            & ' cost_ratio=',REAL(IDIRECT_MATRIX_HYBRID_FACTOR,JPRD)/&
            & REAL(IDIRECT_MATRIX_HYBRID_DENSE,JPRD),&
            & ' mean_contrib=',REAL(IDIRECT_MATRIX_CONTRIB,JPRD)/REAL(ILA,JPRD),&
            & ' max_contrib=',IDIRECT_MATRIX_MAX_CONTRIB
        ENDIF
        IF (BUTTERFLY_TRANSPOSE_DIRECT_REQUESTED()) &
          & CALL CONSTRUCT_TRANSPOSED_HYBRID(ZEPS_INT_DEC,IMAXCOLS,&
          & S%FA(IMLOC)%RPNMDA,S%FA(IMLOC)%YHYBRID_A,&
          & S%FA(IMLOC)%YHYBRID_DIRECT_A)
        DEALLOCATE(S%FA(IMLOC)%RPNMDA)
      ENDIF
      IF (ILS > IHYBRID_MINCOLS) THEN
        ...
        IF (LDIRECT_TRANSPOSE_DIAGNOSTICS) THEN
          CALL ANALYZE_TRANSPOSED_HYBRID(ZEPS_INT_DEC,IMAXCOLS,&
            & S%FA(IMLOC)%RPNMDS,S%FA(IMLOC)%YHYBRID_S,&
            & IDIRECT_MATRIX_HYBRID_DENSE,IDIRECT_MATRIX_HYBRID_FACTOR,&
            & IDIRECT_MATRIX_BLOCKS,IDIRECT_MATRIX_SELECTED,&
            & IDIRECT_MATRIX_CONTRIB,IDIRECT_MATRIX_MAX_CONTRIB)
          IDIRECT_HYBRID_DENSE(2)=IDIRECT_HYBRID_DENSE(2)+&
            & IDIRECT_MATRIX_HYBRID_DENSE
          IDIRECT_HYBRID_FACTOR(2)=IDIRECT_HYBRID_FACTOR(2)+&
            & IDIRECT_MATRIX_HYBRID_FACTOR
          IDIRECT_HYBRID_BLOCKS(2)=IDIRECT_HYBRID_BLOCKS(2)+&
            & IDIRECT_MATRIX_BLOCKS
          IDIRECT_HYBRID_SELECTED(2)=IDIRECT_HYBRID_SELECTED(2)+&
            & IDIRECT_MATRIX_SELECTED
          IDIRECT_HYBRID_CONTRIB(2)=IDIRECT_HYBRID_CONTRIB(2)+&
            & IDIRECT_MATRIX_CONTRIB
          IDIRECT_HYBRID_OUTPUTS(2)=IDIRECT_HYBRID_OUTPUTS(2)+ILS
          IDIRECT_HYBRID_MAX_CONTRIB(2)=MAX(IDIRECT_HYBRID_MAX_CONTRIB(2),&
            & IDIRECT_MATRIX_MAX_CONTRIB)
          WRITE(NOUT,'(A,A,A,I0,A,I0,A,I0,A,I0,A,I0,A,F8.5,A,F8.3,A,I0)') &
            & 'GPU_DIRECT_TRANSPOSE_HYBRID_MATRIX path=','S',' m=',IM,&
            & ' blocks=',IDIRECT_MATRIX_BLOCKS,&
            & ' selected=',IDIRECT_MATRIX_SELECTED,&
            & ' dense_cost=',IDIRECT_MATRIX_HYBRID_DENSE,&
            & ' factor_cost=',IDIRECT_MATRIX_HYBRID_FACTOR,&
            & ' cost_ratio=',REAL(IDIRECT_MATRIX_HYBRID_FACTOR,JPRD)/&
            & REAL(IDIRECT_MATRIX_HYBRID_DENSE,JPRD),&
            & ' mean_contrib=',REAL(IDIRECT_MATRIX_CONTRIB,JPRD)/REAL(ILS,JPRD),&
            & ' max_contrib=',IDIRECT_MATRIX_MAX_CONTRIB
        ENDIF
        IF (BUTTERFLY_TRANSPOSE_DIRECT_REQUESTED()) &
          & CALL CONSTRUCT_TRANSPOSED_HYBRID(ZEPS_INT_DEC,IMAXCOLS,&
          & S%FA(IMLOC)%RPNMDS,S%FA(IMLOC)%YHYBRID_S,&
          & S%FA(IMLOC)%YHYBRID_DIRECT_S)
    ...
    IF (LDIRECT_TRANSPOSE_DIAGNOSTICS) THEN
      DO IPATH=1,2
        ...
        ELSE
          CTRUNC_PATH='S'
        ENDIF
        WRITE(NOUT,'(A,A,A,I0,A,I0,A,I0,A,I0,A,F8.5,A,F8.3,A,I0)') &
          & 'GPU_DIRECT_TRANSPOSE_HYBRID path=',CTRUNC_PATH,&
          & ' blocks=',IDIRECT_HYBRID_BLOCKS(IPATH),&
          & ' selected=',IDIRECT_HYBRID_SELECTED(IPATH),&
          & ' dense_cost=',IDIRECT_HYBRID_DENSE(IPATH),&
          & ' factor_cost=',IDIRECT_HYBRID_FACTOR(IPATH),&
          & ' cost_ratio=',REAL(IDIRECT_HYBRID_FACTOR(IPATH),JPRD)/&
          & REAL(IDIRECT_HYBRID_DENSE(IPATH),JPRD),&
          & ' mean_contrib=',REAL(IDIRECT_HYBRID_CONTRIB(IPATH),JPRD)/&
          & REAL(IDIRECT_HYBRID_OUTPUTS(IPATH),JPRD),&
          & ' max_contrib=',IDIRECT_HYBRID_MAX_CONTRIB(IPATH)
      ENDDO
    ENDIF
  ENDIF
#endif
