...
USE PARKIND_ECTRANS,  ONLY: JPRD, JPIM, JPRBT, JPIB
...
#ifndef TRANS_SINGLE
USE BUTTERFLY_ALG_DP_MOD,ONLY: CONSTRUCT_HYBRID,ANALYZE_LEGENDRE_TRUNCATION,&
  & LEGENDRE_TRUNCATION_DIAGNOSTICS_REQUESTED
USE BUTTERFLY_GPU_DP_MOD,ONLY: BUTTERFLY_REQUESTED
#endif
...
INTEGER(KIND=JPIM) :: INM, IM, IRECV, ISEND, ISREQ, IRREQ, &
             &JGL,  JM, JMLOC, IMLOC, JN, JNM, IODD, INN, INMAX, JI, IMAXN, ITAG, &
             &INX, ISL, ISTART, ITHRESHOLD, INSMAX, IMAXCOLS,IHYBRID_LEAF,ILATSMAX,JW,JV,J, &
...
REAL(KIND=JPRD),ALLOCATABLE :: ZHYBRID_THETA(:)

LOGICAL :: LLP1,LLP2

#ifndef TRANS_SINGLE
INTEGER(KIND=JPIM),PARAMETER :: NTRUNC_EPS=3,NTRUNC_BANDS=6
REAL(KIND=JPRD),PARAMETER :: ZTRUNC_EPS(NTRUNC_EPS)=(/&
  & 1.0E-12_JPRD,1.0E-13_JPRD,1.0E-14_JPRD/)
INTEGER(KIND=JPIM),PARAMETER :: ITRUNC_BANDS(NTRUNC_BANDS)=(/0,1,2,4,8,16/)
INTEGER(KIND=JPIM) :: IBAND,IEPS,IPATH
INTEGER(KIND=JPIB) :: ITRUNC_DENSE(2),ITRUNC_KEEP(2,NTRUNC_EPS,NTRUNC_BANDS)
INTEGER(KIND=JPIB) :: ITRUNC_MATRIX_KEEP(NTRUNC_BANDS)
REAL(KIND=JPRD) :: ZTRUNC_DROP2(2,NTRUNC_EPS,NTRUNC_BANDS),ZTRUNC_TOTAL2(2)
REAL(KIND=JPRD) :: ZTRUNC_MATRIX_DROP2(NTRUNC_BANDS),ZTRUNC_MATRIX_TOTAL2
REAL(KIND=JPRD) :: ZTRUNC_COST_RATIO,ZTRUNC_ERROR
LOGICAL :: LTRUNC_DIAGNOSTICS
CHARACTER(LEN=1) :: CTRUNC_PATH
#endif
CHARACTER(LEN=16) :: CBUTTERFLY_MAXCOLS
INTEGER :: IBUTTERFLY_ENV_LENGTH,IBUTTERFLY_ENV_STATUS
LOGICAL :: LBUTTERFLY_MAXCOLS_SET
...
IHYBRID_LEAF=IMAXCOLS
LBUTTERFLY_MAXCOLS_SET=.FALSE.
#ifndef TRANS_SINGLE
CALL GET_ENVIRONMENT_VARIABLE('ECTRANS_GPU_BUTTERFLY_MAXCOLS',&
  & CBUTTERFLY_MAXCOLS,LENGTH=IBUTTERFLY_ENV_LENGTH,&
  & STATUS=IBUTTERFLY_ENV_STATUS)
IF (IBUTTERFLY_ENV_STATUS == 0 .AND. IBUTTERFLY_ENV_LENGTH > 0) THEN
  READ(CBUTTERFLY_MAXCOLS(1:IBUTTERFLY_ENV_LENGTH),*,&
    & IOSTAT=IBUTTERFLY_ENV_STATUS) IMAXCOLS
  LBUTTERFLY_MAXCOLS_SET=IBUTTERFLY_ENV_STATUS == 0 .AND. IMAXCOLS > 0
  IF (.NOT.LBUTTERFLY_MAXCOLS_SET) IMAXCOLS=IHYBRID_LEAF
ENDIF
#endif
ITHRESHOLD = MAX(ITHRESHOLD,IHYBRID_LEAF+1,IMAXCOLS+1)
S%ITHRESHOLD = ITHRESHOLD
#ifndef TRANS_SINGLE
IF (BUTTERFLY_REQUESTED()) WRITE(NOUT,'(A,I0,A,I0,A,I0)') &
  & 'GPU butterfly maxcols=',IMAXCOLS,' hybrid_leaf=',IHYBRID_LEAF,&
...
#ifndef TRANS_SINGLE
  LTRUNC_DIAGNOSTICS=LEGENDRE_TRUNCATION_DIAGNOSTICS_REQUESTED()
  IF (LTRUNC_DIAGNOSTICS) THEN
    ITRUNC_DENSE=0
    ITRUNC_KEEP=0
    ZTRUNC_DROP2=0.0_JPRD
    ZTRUNC_TOTAL2=0.0_JPRD
    DO IMLOC=1,D%NUMP
      IM=D%MYMS(IMLOC)
      IF (IM == 0) CYCLE
      ILA=(R%NSMAX-IM+2)/2
      ILS=(R%NSMAX-IM+3)/2
      IDGLU=MIN(R%NDGNH,G%NDGLU(IM))
      IF (ILA > 0 .AND. IDGLU > 0) THEN
        ITRUNC_DENSE(1)=ITRUNC_DENSE(1)+INT(IDGLU,JPIB)*INT(ILA,JPIB)
        DO IEPS=1,NTRUNC_EPS
          CALL ANALYZE_LEGENDRE_TRUNCATION(&
            & REAL(S%FA(IMLOC)%RPNMA(:,:),JPRD),ZTRUNC_EPS(IEPS),ITRUNC_BANDS,&
            & ITRUNC_MATRIX_KEEP,ZTRUNC_MATRIX_DROP2,ZTRUNC_MATRIX_TOTAL2)
          ITRUNC_KEEP(1,IEPS,:)=ITRUNC_KEEP(1,IEPS,:)+ITRUNC_MATRIX_KEEP
          ZTRUNC_DROP2(1,IEPS,:)=ZTRUNC_DROP2(1,IEPS,:)+ZTRUNC_MATRIX_DROP2
        ENDDO
        ZTRUNC_TOTAL2(1)=ZTRUNC_TOTAL2(1)+ZTRUNC_MATRIX_TOTAL2
      ENDIF
      IF (ILS > 0 .AND. IDGLU > 0) THEN
        ITRUNC_DENSE(2)=ITRUNC_DENSE(2)+INT(IDGLU,JPIB)*INT(ILS,JPIB)
        DO IEPS=1,NTRUNC_EPS
          CALL ANALYZE_LEGENDRE_TRUNCATION(&
            & REAL(S%FA(IMLOC)%RPNMS(:,:),JPRD),ZTRUNC_EPS(IEPS),ITRUNC_BANDS,&
            & ITRUNC_MATRIX_KEEP,ZTRUNC_MATRIX_DROP2,ZTRUNC_MATRIX_TOTAL2)
          ITRUNC_KEEP(2,IEPS,:)=ITRUNC_KEEP(2,IEPS,:)+ITRUNC_MATRIX_KEEP
          ZTRUNC_DROP2(2,IEPS,:)=ZTRUNC_DROP2(2,IEPS,:)+ZTRUNC_MATRIX_DROP2
        ENDDO
        ZTRUNC_TOTAL2(2)=ZTRUNC_TOTAL2(2)+ZTRUNC_MATRIX_TOTAL2
      ENDIF
    ENDDO
    ...
    DO IPATH=1,2
      ...
      ENDIF
      DO IEPS=1,NTRUNC_EPS
        DO IBAND=1,NTRUNC_BANDS
          ZTRUNC_COST_RATIO=REAL(ITRUNC_KEEP(IPATH,IEPS,IBAND),JPRD)/&
            & REAL(ITRUNC_DENSE(IPATH),JPRD)
          ZTRUNC_ERROR=SQRT(ZTRUNC_DROP2(IPATH,IEPS,IBAND)/ZTRUNC_TOTAL2(IPATH))
          WRITE(NOUT,'(A,A,A,ES10.2,A,I0,A,I0,A,I0,A,F8.5,A,ES12.4)') &
            & 'GPU_TRUNCATION path=',CTRUNC_PATH,' eps=',ZTRUNC_EPS(IEPS),&
            & ' bands=',ITRUNC_BANDS(IBAND),' dense_cost=',ITRUNC_DENSE(IPATH),&
            & ' kept_cost=',ITRUNC_KEEP(IPATH,IEPS,IBAND),&
            & ' cost_ratio=',ZTRUNC_COST_RATIO,' discarded_frob=',ZTRUNC_ERROR
        ENDDO
      ENDDO
    ENDDO
  ENDIF

  IF (BUTTERFLY_REQUESTED()) THEN
    DO IMLOC=1,D%NUMP
      IM = D%MYMS(IMLOC)
      IF (IM == 0) CYCLE
      ISL = MAX(R%NDGNH-G%NDGLU(IM)+1,1)
      ...
      ILS = (R%NSMAX-IM+3)/2
      IDGLU = MIN(R%NDGNH,G%NDGLU(IM))
      ALLOCATE(ZHYBRID_THETA(IDGLU))
      ZHYBRID_THETA=ACOS(MAX(MIN(F%RMU(ISL:ISL+IDGLU-1),1.0_JPRD),-1.0_JPRD))
      IF (ILA > ITHRESHOLD) THEN
        ALLOCATE(S%FA(IMLOC)%RPNMDA(IDGLU,ILA))
        S%FA(IMLOC)%RPNMDA(:,:) = REAL(S%FA(IMLOC)%RPNMA(:,:),JPRD)
        CALL CONSTRUCT_HYBRID(ZEPS_INT_DEC,IMAXCOLS,IHYBRID_LEAF,IM,&
          & R%NSMAX+2-IA,'A',NOUT,ZHYBRID_THETA,S%FA(IMLOC)%RPNMDA,&
          & S%FA(IMLOC)%YHYBRID_A)
        DEALLOCATE(S%FA(IMLOC)%RPNMDA)
      ENDIF
      IF (ILS > ITHRESHOLD) THEN
        ALLOCATE(S%FA(IMLOC)%RPNMDS(IDGLU,ILS))
        S%FA(IMLOC)%RPNMDS(:,:) = REAL(S%FA(IMLOC)%RPNMS(:,:),JPRD)
        CALL CONSTRUCT_HYBRID(ZEPS_INT_DEC,IMAXCOLS,IHYBRID_LEAF,IM,&
          & R%NSMAX+2-IS,'S',NOUT,ZHYBRID_THETA,S%FA(IMLOC)%RPNMDS,&
          & S%FA(IMLOC)%YHYBRID_S)
        DEALLOCATE(S%FA(IMLOC)%RPNMDS)
      ENDIF
      DEALLOCATE(ZHYBRID_THETA)
    ENDDO
  ENDIF
#endif
