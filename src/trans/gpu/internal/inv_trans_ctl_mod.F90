...
CONTAINS
  SUBROUTINE INV_TRANS_CTL(KF_UV_G,KF_SCALARS_G,KF_GP,KF_FS,KF_OUT_LT,&
    ...
    & PSPSC3A,PSPSC3B,PSPSC2,KVSETSC3A,KVSETSC3B,KVSETSC2,PGPUV,PGP3A,PGP3B,PGP2,LDUPDATE_HOST)
    ...
    ! 260420 wrqt begin
    ! 260420 wrqt comment输出GPU逆变换控制层的关键尺寸与偏移，便于核对各类场在Legendre与Fourier阶段的布局
    IF (NPRINTLEV > 0 .AND. MYPROC == 1 .AND. .NOT. LREPORTED) THEN
      WRITE(NOUT,'(A,8(I0,1X))') 'TRACE GPU INV_TRANS_CTL: KF_UV_G KF_SCALARS_G KF_GP KF_FS KF_OUT_LT KF_UV KF_SCALARS KF_SCDERS = ', &
        & KF_UV_G, KF_SCALARS_G, KF_GP, KF_FS, KF_OUT_LT, KF_UV, KF_SCALARS, KF_SCDERS
      WRITE(NOUT,'(A,7(I0,1X))') 'TRACE GPU INV_TRANS_CTL: KUV_OFFSET KSCALARS_OFFSET KSCALARS_NSDER_OFFSET KUV_EWDER_OFFSET KSCALARS_EWDER_OFFSET IF_LEG IF_FOURIER = ', &
        & KUV_OFFSET, KSCALARS_OFFSET, KSCALARS_NSDER_OFFSET, KUV_EWDER_OFFSET, KSCALARS_EWDER_OFFSET, IF_LEG, IF_FOURIER
      WRITE(NOUT,'(A,4(L1,1X))') 'TRACE GPU INV_TRANS_CTL: LVORGP LDIVGP LUVDER LSCDERS = ', &
        & LVORGP, LDIVGP, LUVDER, LSCDERS
      LREPORTED = .TRUE.
    ENDIF
    ! 260420 wrqt end
    ...
    IF (KF_FS > 0) THEN
      ...
      CALL REPORT_WARMUP_STAGE('ltinv')
      CALL GSTATS(102,1)
      ...
      CALL REPORT_WARMUP_STAGE('ftinv')
      CALL GSTATS(107,1)
    ENDIF
    ...
    CALL TRLTOG(ALLOCATOR,HTRLTOG,PREEL_REAL,IF_FOURIER,KF_GP,KF_UV_G,KF_SCALARS_G,&
     ...
     & PGP=PGP,PGPUV=PGPUV,PGP3A=PGP3A,PGP3B=PGP3B,PGP2=PGP2,LDUPDATE_HOST=LDUPDATE_HOST)
    CALL REPORT_WARMUP_STAGE('l2g')
    CALL GSTATS(157,1)
  ...
  CONTAINS
    SUBROUTINE REPORT_WARMUP_STAGE(CDSTAGE)
      ...
      WRITE(NOUT,'(A,A,A,I0,A,I0,A,F10.3)') &
        & 'EC_WARMUP_DEBUG event=transform_stage direction=inv stage=',TRIM(CDSTAGE), &
