...
CONTAINS
  ...
  SUBROUTINE LEDIR(ALLOCATOR,ZINPS,ZINPA,ZINPS0,ZINPA0,ZOUT,ZOUT0,POA1,KF_FS)
    ...
    USE BUTTERFLY_GPU_DP_MOD,        ONLY: APPLY_BUTTERFLY_DIRECT,&
      ...
      & BUTTERFLY_DIRECT_OVERLAP_REQUESTED
#endif
...
#define HIP_GEMM HIP_DGEMM_GROUPED
    ...
    INTEGER(KIND=JPIM) :: IA, IS, J, IBATCH, IGROUP, IMLOC0(1)
#ifndef TRANS_SINGLE
    INTEGER(KIND=JPIM)  :: KS(MAX(D%NUMP,S%PLAN_DIRECT_A%DIRECT_GROUP_COUNT,&
      & S%PLAN_DIRECT_S%DIRECT_GROUP_COUNT))
    INTEGER(KIND=JPIM)  :: NS(MAX(D%NUMP,S%PLAN_DIRECT_A%DIRECT_GROUP_COUNT,&
      & S%PLAN_DIRECT_S%DIRECT_GROUP_COUNT))
    INTEGER(KIND=JPIM)  :: LDBS(MAX(D%NUMP,S%PLAN_DIRECT_A%DIRECT_GROUP_COUNT,&
      & S%PLAN_DIRECT_S%DIRECT_GROUP_COUNT))
    INTEGER(KIND=JPIB)  :: AOFFSETS(MAX(D%NUMP,S%PLAN_DIRECT_A%DIRECT_GROUP_COUNT,&
      & S%PLAN_DIRECT_S%DIRECT_GROUP_COUNT))
    INTEGER(KIND=JPIB)  :: BOFFSETS(MAX(D%NUMP,S%PLAN_DIRECT_A%DIRECT_GROUP_COUNT,&
      & S%PLAN_DIRECT_S%DIRECT_GROUP_COUNT))
    INTEGER(KIND=JPIB)  :: COFFSETS(MAX(D%NUMP,S%PLAN_DIRECT_A%DIRECT_GROUP_COUNT,&
      & S%PLAN_DIRECT_S%DIRECT_GROUP_COUNT))
#else
    INTEGER(KIND=JPIM)  :: KS(D%NUMP), NS(D%NUMP)
    INTEGER(KIND=JPIM)  :: LDBS(D%NUMP)
    ...
    LUSE_BUTTERFLY_TRANSPOSE_DIRECT=LUSE_BUTTERFLY_DIRECT .AND. &
      & BUTTERFLY_TRANSPOSE_DIRECT_REQUESTED()
    LOVERLAP_BUTTERFLY_DIRECT=LUSE_BUTTERFLY_DIRECT .AND. &
      & BUTTERFLY_DIRECT_OVERLAP_REQUESTED()
#ifndef ACCGPU
    LOVERLAP_BUTTERFLY_DIRECT=.FALSE.
#endif
#endif
    ...
    IF (LUSE_BUTTERFLY_TRANSPOSE_DIRECT .AND. &
      & .NOT.S%PLAN_DIRECT_A%DIRECT_INLINE_CROP .AND. &
      & .NOT.S%PLAN_DIRECT_A%DIRECT_NATIVE_BLOCKS) THEN
      CALL APPLY_BUTTERFLY_DIRECT_FORWARD(S%PLAN_DIRECT_A,ZINPA,ZOUT,&
        & IIN_STRIDES0,D_OFFSETS_GEMM2(D_NUMP+1),&
        & MERGE(2,1,LOVERLAP_BUTTERFLY_DIRECT),2110,ALLOCATOR)
    ELSEIF (LUSE_BUTTERFLY_DIRECT .AND. &
      & .NOT.LUSE_BUTTERFLY_TRANSPOSE_DIRECT) THEN
      CALL APPLY_BUTTERFLY_DIRECT(S%PLAN_A,ZINPA,ZOUT,IIN_STRIDES0,&
        & D_OFFSETS_GEMM2(D_NUMP+1),MERGE(2,1,LOVERLAP_BUTTERFLY_DIRECT),&
        & 2110,ALLOCATOR)
    ENDIF
#endif
...
#ifndef TRANS_SINGLE
    IF (LUSE_BUTTERFLY_TRANSPOSE_DIRECT .AND. &
      & S%PLAN_DIRECT_A%DIRECT_NATIVE_BLOCKS) THEN
      IBATCH=S%PLAN_DIRECT_A%DIRECT_GROUP_COUNT
      DO IGROUP=1,IBATCH
        KMLOC=S%PLAN_DIRECT_A%DIRECT_GROUP_MLOC(IGROUP)
        NS(IGROUP)=S%PLAN_DIRECT_A%DIRECT_GROUP_N(IGROUP)
        KS(IGROUP)=S%PLAN_DIRECT_A%DIRECT_GROUP_K(IGROUP)
        LDBS(IGROUP)=D%LEGENDRE_MATRIX_STRIDES(KMLOC)
        AOFFSETS(IGROUP)=IIN_STRIDES0*INT(D_OFFSETS_GEMM1(KMLOC)+&
          & S%PLAN_DIRECT_A%DIRECT_GROUP_FIRST_COL(IGROUP)-1,JPIB)
        BOFFSETS(IGROUP)=D%OFFSETS_GEMM_MATRIX(KMLOC)+&
          & S%PLAN_DIRECT_A%DIRECT_GROUP_FIRST_COL(IGROUP)-1+&
          & INT(S%PLAN_DIRECT_A%DIRECT_GROUP_FIRST_ROW(IGROUP)-1,JPIB)*&
          & D%LEGENDRE_MATRIX_STRIDES(KMLOC)
        COFFSETS(IGROUP)=IOUT_STRIDES0*INT(D_OFFSETS_GEMM2(KMLOC)+&
          & S%PLAN_DIRECT_A%DIRECT_GROUP_FIRST_ROW(IGROUP)-1,JPIB)
      ENDDO
    ELSE
#endif
    DO KMLOC=1,D_NUMP
      ...
      KS(KMLOC) = G_NDGLU(KM)
      LDBS(KMLOC) = D%LEGENDRE_MATRIX_STRIDES(KMLOC)
      ...
      IF (LUSE_BUTTERFLY_TRANSPOSE_DIRECT .AND. &
        ...
        IF (S%PLAN_DIRECT_A%DIRECT_INLINE_CROP) THEN
          AOFFSETS(KMLOC)=AOFFSETS(KMLOC)+IIN_STRIDES0*&
            & INT(S%PLAN_DIRECT_A%DIRECT_CROP_FIRST(KMLOC)-1,JPIB)
          BOFFSETS(KMLOC)=BOFFSETS(KMLOC)+&
            & S%PLAN_DIRECT_A%DIRECT_CROP_FIRST(KMLOC)-1
          KS(KMLOC)=S%PLAN_DIRECT_A%DIRECT_CROP_COUNT(KMLOC)
        ELSE
          NS(KMLOC)=0
          KS(KMLOC)=0
        ENDIF
    ...
    CALL HIP_GEMM( &
      ...
      & C_LOC(ZAA), LDBS, BOFFSETS, &
      & 0.0_JPRBT, &
      & C_LOC(ZOUT), IOUT_STRIDES0, COFFSETS, &
      & IBATCH, HIP_STREAM, C_LOC(ALLOCATOR%PTR))
#ifdef OMPGPU
    !$OMP END TARGET DATA
#endif
#ifdef ACCGPU
...
#ifndef TRANS_SINGLE
#ifdef ACCGPU
    IF (LOVERLAP_BUTTERFLY_DIRECT) THEN
      !$ACC WAIT(2) ASYNC(1)
    ENDIF
#endif
#endif
...
#ifndef TRANS_SINGLE
    IF (LUSE_BUTTERFLY_TRANSPOSE_DIRECT .AND. &
      & .NOT.S%PLAN_DIRECT_S%DIRECT_INLINE_CROP .AND. &
      & .NOT.S%PLAN_DIRECT_S%DIRECT_NATIVE_BLOCKS) THEN
      CALL APPLY_BUTTERFLY_DIRECT_FORWARD(S%PLAN_DIRECT_S,ZINPS,ZOUT,&
        & IIN_STRIDES0,D_OFFSETS_GEMM2(D_NUMP+1),&
        & MERGE(2,1,LOVERLAP_BUTTERFLY_DIRECT),2210,ALLOCATOR)
    ELSEIF (LUSE_BUTTERFLY_DIRECT .AND. &
      & .NOT.LUSE_BUTTERFLY_TRANSPOSE_DIRECT) THEN
...
#ifndef TRANS_SINGLE
    IF (LUSE_BUTTERFLY_TRANSPOSE_DIRECT .AND. &
      & S%PLAN_DIRECT_S%DIRECT_NATIVE_BLOCKS) THEN
      IBATCH=S%PLAN_DIRECT_S%DIRECT_GROUP_COUNT
      DO IGROUP=1,IBATCH
        KMLOC=S%PLAN_DIRECT_S%DIRECT_GROUP_MLOC(IGROUP)
        NS(IGROUP)=S%PLAN_DIRECT_S%DIRECT_GROUP_N(IGROUP)
        KS(IGROUP)=S%PLAN_DIRECT_S%DIRECT_GROUP_K(IGROUP)
        LDBS(IGROUP)=D%LEGENDRE_MATRIX_STRIDES(KMLOC)
        AOFFSETS(IGROUP)=IIN_STRIDES0*INT(D_OFFSETS_GEMM1(KMLOC)+&
          & S%PLAN_DIRECT_S%DIRECT_GROUP_FIRST_COL(IGROUP)-1,JPIB)
        BOFFSETS(IGROUP)=D%OFFSETS_GEMM_MATRIX(KMLOC)+&
          & S%PLAN_DIRECT_S%DIRECT_GROUP_FIRST_COL(IGROUP)-1+&
          & INT(S%PLAN_DIRECT_S%DIRECT_GROUP_FIRST_ROW(IGROUP)-1,JPIB)*&
          & D%LEGENDRE_MATRIX_STRIDES(KMLOC)
        COFFSETS(IGROUP)=IOUT_STRIDES0*INT(D_OFFSETS_GEMM2(KMLOC)+&
          & S%PLAN_DIRECT_S%DIRECT_GROUP_FIRST_ROW(IGROUP)-1,JPIB)
      ENDDO
    ELSE
#endif
    DO KMLOC=1,D_NUMP
      ...
      IF (LUSE_BUTTERFLY_TRANSPOSE_DIRECT .AND. &
        ...
        IF (S%PLAN_DIRECT_S%DIRECT_INLINE_CROP) THEN
          AOFFSETS(KMLOC)=AOFFSETS(KMLOC)+IIN_STRIDES0*&
            & INT(S%PLAN_DIRECT_S%DIRECT_CROP_FIRST(KMLOC)-1,JPIB)
          BOFFSETS(KMLOC)=BOFFSETS(KMLOC)+&
            & S%PLAN_DIRECT_S%DIRECT_CROP_FIRST(KMLOC)-1
          KS(KMLOC)=S%PLAN_DIRECT_S%DIRECT_CROP_COUNT(KMLOC)
        ELSE
          NS(KMLOC)=0
          KS(KMLOC)=0
        ENDIF
