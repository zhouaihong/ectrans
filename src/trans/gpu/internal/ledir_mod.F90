...
#ifndef TRANS_SINGLE
      IOUT_SIZE = IOUT_SIZE+2*IOUT_STRIDES0*BUTTERFLY_WORK_COLUMNS()
#endif
    ENDIF
...
#ifndef TRANS_SINGLE
    USE TPM_FLT,                     ONLY: S
    USE BUTTERFLY_GPU_DP_MOD,        ONLY: APPLY_BUTTERFLY_DIRECT,&
      & APPLY_BUTTERFLY_DIRECT_FORWARD,BUTTERFLY_DIRECT_REQUESTED,&
...
#ifndef TRANS_SINGLE
    LOGICAL :: LUSE_BUTTERFLY_DIRECT,LUSE_BUTTERFLY_TRANSPOSE_DIRECT
#endif
...
#ifndef TRANS_SINGLE
    LUSE_BUTTERFLY_DIRECT=BUTTERFLY_DIRECT_REQUESTED()
    LUSE_BUTTERFLY_TRANSPOSE_DIRECT=LUSE_BUTTERFLY_DIRECT .AND. &
      & BUTTERFLY_TRANSPOSE_DIRECT_REQUESTED()
#endif
...
#ifndef TRANS_SINGLE
    IF (LUSE_BUTTERFLY_TRANSPOSE_DIRECT) THEN
      CALL APPLY_BUTTERFLY_DIRECT_FORWARD(S%PLAN_DIRECT_A,ZINPA,ZOUT,&
        & IIN_STRIDES0,D_OFFSETS_GEMM2(D_NUMP+1),1,2110,ALLOCATOR)
    ELSEIF (LUSE_BUTTERFLY_DIRECT) THEN
      CALL APPLY_BUTTERFLY_DIRECT(S%PLAN_A,ZINPA,ZOUT,IIN_STRIDES0,&
        & D_OFFSETS_GEMM2(D_NUMP+1),1,2110,ALLOCATOR)
    ENDIF
#endif
...
#ifndef TRANS_SINGLE
      IF (LUSE_BUTTERFLY_TRANSPOSE_DIRECT .AND. &
        & S%PLAN_DIRECT_A%ENABLED .AND. &
        & S%PLAN_DIRECT_A%USE_HYBRID(KMLOC)) THEN
        NS(KMLOC)=0
        KS(KMLOC)=0
      ELSEIF (LUSE_BUTTERFLY_DIRECT .AND. S%PLAN_A%ENABLED .AND. &
        & S%PLAN_A%USE_HYBRID(KMLOC)) THEN
        NS(KMLOC)=0
        KS(KMLOC)=0
      ENDIF
#endif
...
#ifndef TRANS_SINGLE
    IF (LUSE_BUTTERFLY_TRANSPOSE_DIRECT) THEN
      CALL APPLY_BUTTERFLY_DIRECT_FORWARD(S%PLAN_DIRECT_S,ZINPS,ZOUT,&
        & IIN_STRIDES0,D_OFFSETS_GEMM2(D_NUMP+1),1,2210,ALLOCATOR)
    ELSEIF (LUSE_BUTTERFLY_DIRECT) THEN
      CALL APPLY_BUTTERFLY_DIRECT(S%PLAN_S,ZINPS,ZOUT,IIN_STRIDES0,&
        & D_OFFSETS_GEMM2(D_NUMP+1),1,2210,ALLOCATOR)
    ENDIF
#endif
...
#ifndef TRANS_SINGLE
      IF (LUSE_BUTTERFLY_TRANSPOSE_DIRECT .AND. &
        & S%PLAN_DIRECT_S%ENABLED .AND. &
        & S%PLAN_DIRECT_S%USE_HYBRID(KMLOC)) THEN
        NS(KMLOC)=0
        KS(KMLOC)=0
      ELSEIF (LUSE_BUTTERFLY_DIRECT .AND. S%PLAN_S%ENABLED .AND. &
        & S%PLAN_S%USE_HYBRID(KMLOC)) THEN
        NS(KMLOC)=0
        KS(KMLOC)=0
      ENDIF
#endif
