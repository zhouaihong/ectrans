...
CONTAINS
  ...
  SUBROUTINE LEINV(ALLOCATOR,PIA,ZINP,ZINP0,ZOUTS,ZOUTA,ZOUTS0,ZOUTA0,KF_LEG)
    ...
    USE LEGENDRE_RECURRENCE_GPU_DP_MOD, ONLY: APPLY_LEGENDRE_RECURRENCE_INVERSE,&
      & LEGENDRE_RECURRENCE_INVERSE_REQUESTED
#endif
...
#define HIP_GEMM HIP_DGEMM_GROUPED
    ...
    CALL LEINV_STRIDES(KF_LEG,IOUT_STRIDES0,IOUT_SIZE,IIN_STRIDES0,IIN_SIZE,&
                       IOUT0_STRIDES0,IOUT0_SIZE,IIN0_STRIDES0,IIN0_SIZE)
#ifndef TRANS_SINGLE
    LUSE_RECURRENCE=LEGENDRE_RECURRENCE_INVERSE_REQUESTED()
#endif
   ...
   ENDIF
...
#ifndef TRANS_SINGLE
    IF (LUSE_RECURRENCE) THEN
      CALL APPLY_LEGENDRE_RECURRENCE_INVERSE(ZAA,ZINP,ZOUTA,IIN_STRIDES0,&
        & 2*KF_LEG,.FALSE.,1)
    ELSE
...
#ifndef TRANS_SINGLE
      IF (LUSE_RECURRENCE .AND. KM /= 0) THEN
        NS(KMLOC)=0
        KS(KMLOC)=0
      ELSEIF (S%PLAN_A%ENABLED .AND. S%PLAN_A%USE_HYBRID(KMLOC)) THEN
        NS(KMLOC)=0
        KS(KMLOC)=0
      ENDIF
#endif
...
#ifndef TRANS_SINGLE
    IF (LUSE_RECURRENCE) THEN
      CALL APPLY_LEGENDRE_RECURRENCE_INVERSE(ZAS,ZINP,ZOUTS,IIN_STRIDES0,&
        & 2*KF_LEG,.TRUE.,1)
    ELSE
    ...
    DO KMLOC=1,D_NUMP
      ...
      IF (LUSE_RECURRENCE .AND. KM /= 0) THEN
        NS(KMLOC)=0
        KS(KMLOC)=0
      ELSEIF (S%PLAN_S%ENABLED .AND. S%PLAN_S%USE_HYBRID(KMLOC)) THEN
        NS(KMLOC)=0
        KS(KMLOC)=0
      ENDIF
#endif
