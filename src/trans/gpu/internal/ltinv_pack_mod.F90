...
CONTAINS
  ...
  SUBROUTINE LTINV_PACK_UV_PAIR(ZINPA,ZINPA0,ZINPS,ZINPS0,PSPVOR,PSPDIV,PSPEC,&
                              & KFIELDS,KF_SPEC,KOUT_U,KOUT_V,KOUT_SPEC,&
                              & PEPSNM,KIIN_STRIDES0,KIIN0_STRIDES0)
    USE PARKIND_ECTRANS, ONLY: JPIM, JPIB, JPRB, JPRBT, JPRD
    USE TPM_DIM,         ONLY: R
    USE TPM_DISTR,       ONLY: D
    ...
    REAL(KIND=JPRBT), INTENT(INOUT) :: ZINPA(:), ZINPS(:)
    REAL(KIND=JPRD),  INTENT(INOUT) :: ZINPA0(:), ZINPS0(:)
    REAL(KIND=JPRB),  INTENT(IN)    :: PSPVOR(:,:), PSPDIV(:,:), PSPEC(:,:)
    REAL(KIND=JPRBT), INTENT(IN)    :: PEPSNM(1:D%NUMP,0:R%NTMAX+2)
    INTEGER(KIND=JPIM), INTENT(IN)  :: KFIELDS, KF_SPEC, KOUT_U, KOUT_V, KOUT_SPEC
    INTEGER(KIND=JPIM), INTENT(IN)  :: KIIN_STRIDES0, KIIN0_STRIDES0

    INTEGER(KIND=JPIM) :: KMLOC, JFLD, J, KM, ILEN, IPAR, JN, ILOOP_FIELDS
    ...
    ILOOP_FIELDS = MAX(KFIELDS,KF_SPEC)

#ifdef ACCGPU
    !$ACC DATA PRESENT(D,D_MYMS,D_NASM0,D_OFFSETS_GEMM2,R,R_NSMAX,R_NTMAX,F,F_RLAPIN) &
    !$ACC&     PRESENT(ZINPA,ZINPA0,ZINPS,ZINPS0,PSPVOR,PSPDIV,PSPEC,PEPSNM) ASYNC(1)
#endif
#ifdef OMPGPU
    !$OMP TARGET DATA MAP(PRESENT,ALLOC:D,D_MYMS,D_NASM0,D_OFFSETS_GEMM2,R,R_NSMAX,R_NTMAX,F,F_RLAPIN) &
    !$OMP&            MAP(PRESENT,ALLOC:ZINPA,ZINPA0,ZINPS,ZINPS0,PSPVOR,PSPDIV,PSPEC,PEPSNM)
#endif

#ifdef OMPGPU
    !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(3) DEFAULT(NONE) &
    !$OMP& PRIVATE(KM,ILEN,IPAR,JN,INM,IOUTR,IPOS,ZKM,SPEC_R,SPEC_I) &
    !$OMP& PRIVATE(VOR_R,VOR_I,DIV_R,DIV_I,VOR_R_M1,VOR_I_M1,VOR_R_P1,VOR_I_P1) &
    !$OMP& PRIVATE(DIV_R_M1,DIV_I_M1,DIV_R_P1,DIV_I_P1,U_R,U_I,V_R,V_I) &
    !$OMP& SHARED(D_NUMP,D_MYMS,D_NASM0,D_OFFSETS_GEMM2,R_NSMAX,R_NTMAX,F_RLAPIN) &
    !$OMP& SHARED(PSPVOR,PSPDIV,PSPEC,PEPSNM,ZINPA,ZINPA0,ZINPS,ZINPS0) &
    !$OMP& FIRSTPRIVATE(KFIELDS,KF_SPEC,KOUT_U,KOUT_V,KOUT_SPEC,ILOOP_FIELDS, &
    !$OMP&              KIIN_STRIDES0,KIIN0_STRIDES0)
#endif
#ifdef ACCGPU
    !$ACC PARALLEL LOOP COLLAPSE(3) DEFAULT(NONE) &
    !$ACC& PRIVATE(KM,ILEN,IPAR,JN,INM,IOUTR,IPOS,ZKM,SPEC_R,SPEC_I) &
    !$ACC& PRIVATE(VOR_R,VOR_I,DIV_R,DIV_I,VOR_R_M1,VOR_I_M1,VOR_R_P1,VOR_I_P1) &
    !$ACC& PRIVATE(DIV_R_M1,DIV_I_M1,DIV_R_P1,DIV_I_P1,U_R,U_I,V_R,V_I) &
    !$ACC& FIRSTPRIVATE(KFIELDS,KF_SPEC,KOUT_U,KOUT_V,KOUT_SPEC,ILOOP_FIELDS, &
