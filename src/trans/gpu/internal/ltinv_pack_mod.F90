...
CONTAINS
  ...
  SUBROUTINE LTINV_PACK_UV_PAIR(ZINPA,ZINPA0,ZINPS,ZINPS0,PSPVOR,PSPDIV,KFIELDS,&
                              & KOUT_VOR,KOUT_DIV,KOUT_U,KOUT_V,LVORGP,LDIVGP,&
                              & PEPSNM,KIIN_STRIDES0,KIIN0_STRIDES0)
    USE PARKIND_ECTRANS, ONLY: JPIM, JPIB, JPRB, JPRBT, JPRD
    USE TPM_DIM,         ONLY: R
    USE TPM_DISTR,       ONLY: D
    ...
    REAL(KIND=JPRB),  INTENT(IN)    :: PSPVOR(:,:), PSPDIV(:,:)
    REAL(KIND=JPRBT), INTENT(IN)    :: PEPSNM(1:D%NUMP,0:R%NTMAX+2)
    INTEGER(KIND=JPIM), INTENT(IN)  :: KFIELDS, KOUT_VOR, KOUT_DIV, KOUT_U, KOUT_V
    INTEGER(KIND=JPIM), INTENT(IN)  :: KIIN_STRIDES0, KIIN0_STRIDES0
    LOGICAL, INTENT(IN)             :: LVORGP, LDIVGP
    ...
    !$OMP& FIRSTPRIVATE(KFIELDS,KOUT_VOR,KOUT_DIV,KOUT_U,KOUT_V,LVORGP,LDIVGP, &
    !$OMP&              KIIN_STRIDES0,KIIN0_STRIDES0)
#endif
#ifdef ACCGPU
    !$ACC PARALLEL LOOP COLLAPSE(3) DEFAULT(NONE) &
    ...
    !$ACC& PRIVATE(VOR_R,VOR_I,DIV_R,DIV_I,VOR_R_M1,VOR_I_M1,VOR_R_P1,VOR_I_P1) &
    !$ACC& PRIVATE(DIV_R_M1,DIV_I_M1,DIV_R_P1,DIV_I_P1,U_R,U_I,V_R,V_I) &
    !$ACC& FIRSTPRIVATE(KFIELDS,KOUT_VOR,KOUT_DIV,KOUT_U,KOUT_V,LVORGP,LDIVGP, &
    !$ACC&              KIIN_STRIDES0,KIIN0_STRIDES0) &
#ifndef _CRAYFTN
    !$ACC& ASYNC(1)
#else
    !$ACC&
      ...
      DO J=1,(R_NSMAX+3)/2
        DO JFLD=1,KFIELDS
          ...
          IF (J <= ILEN) THEN
            ...
            IF (KM /= 0) THEN
              IF (LVORGP) THEN
                IOUTR = 2*(KOUT_VOR+JFLD)-1
                IPOS = IOUTR+(J-1)*KIIN_STRIDES0+D_OFFSETS_GEMM2(KMLOC)*KIIN_STRIDES0
                ZINPA(IPOS) = REAL(VOR_R,JPRBT)
                ZINPA(IPOS+1) = REAL(VOR_I,JPRBT)
              ENDIF
              IF (LDIVGP) THEN
                IOUTR = 2*(KOUT_DIV+JFLD)-1
                IPOS = IOUTR+(J-1)*KIIN_STRIDES0+D_OFFSETS_GEMM2(KMLOC)*KIIN_STRIDES0
                ZINPA(IPOS) = REAL(DIV_R,JPRBT)
                ZINPA(IPOS+1) = REAL(DIV_I,JPRBT)
              ENDIF
              IOUTR = 2*(KOUT_U+JFLD)-1
              IPOS = IOUTR+(J-1)*KIIN_STRIDES0+D_OFFSETS_GEMM2(KMLOC)*KIIN_STRIDES0
            ...
            ELSE
              IF (LVORGP) THEN
                IPOS = KOUT_VOR+JFLD+(J-1)*KIIN0_STRIDES0
                ZINPA0(IPOS) = REAL(VOR_R,JPRD)
              ENDIF
              IF (LDIVGP) THEN
                IPOS = KOUT_DIV+JFLD+(J-1)*KIIN0_STRIDES0
                ZINPA0(IPOS) = REAL(DIV_R,JPRD)
              ENDIF
              IPOS = KOUT_U+JFLD+(J-1)*KIIN0_STRIDES0
          ...
          IF (J <= ILEN) THEN
            ...
            IF (KM /= 0) THEN
              IF (LVORGP) THEN
                IOUTR = 2*(KOUT_VOR+JFLD)-1
                IPOS = IOUTR+(J-1)*KIIN_STRIDES0+D_OFFSETS_GEMM2(KMLOC)*KIIN_STRIDES0
                ZINPS(IPOS) = REAL(VOR_R,JPRBT)
                ZINPS(IPOS+1) = REAL(VOR_I,JPRBT)
              ENDIF
              IF (LDIVGP) THEN
                IOUTR = 2*(KOUT_DIV+JFLD)-1
                IPOS = IOUTR+(J-1)*KIIN_STRIDES0+D_OFFSETS_GEMM2(KMLOC)*KIIN_STRIDES0
                ZINPS(IPOS) = REAL(DIV_R,JPRBT)
                ZINPS(IPOS+1) = REAL(DIV_I,JPRBT)
              ENDIF
              IOUTR = 2*(KOUT_U+JFLD)-1
              IPOS = IOUTR+(J-1)*KIIN_STRIDES0+D_OFFSETS_GEMM2(KMLOC)*KIIN_STRIDES0
            ...
            ELSE
              IF (LVORGP) THEN
                IPOS = KOUT_VOR+JFLD+(J-1)*KIIN0_STRIDES0
                ZINPS0(IPOS) = REAL(VOR_R,JPRD)
              ENDIF
              IF (LDIVGP) THEN
                IPOS = KOUT_DIV+JFLD+(J-1)*KIIN0_STRIDES0
                ZINPS0(IPOS) = REAL(DIV_R,JPRD)
              ENDIF
              IPOS = KOUT_U+JFLD+(J-1)*KIIN0_STRIDES0
