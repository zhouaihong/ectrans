...
USE TRGTOL_MOD,      ONLY: CLEAN_TRGTOL_CACHE
...
!$ACC EXIT DATA DELETE(FG%ZAA0,FG%ZAS0) IF(ALLOCATED(FG%ZAA0)) ASYNC(1)
!$ACC EXIT DATA DELETE(FG%ZREC_A1,FG%ZREC_A2,FG%ZREC_A3,FG%ZREC_S1,FG%ZREC_S2,FG%ZREC_S3) &
!$ACC& IF(ALLOCATED(FG%ZREC_A1)) ASYNC(1)
!$ACC EXIT DATA DELETE(FG%ZREC_MU2,FG%IREC_A_SEED,FG%IREC_S_SEED) &
!$ACC& IF(ALLOCATED(FG%ZREC_MU2)) ASYNC(1)
!$ACC EXIT DATA DELETE(FG%IREC_TILE_MLOC,FG%IREC_TILE_ROW) &
!$ACC& IF(ALLOCATED(FG%IREC_TILE_MLOC)) ASYNC(1)
!$ACC EXIT DATA DELETE(FG%ZAA,FG%ZAS,FG%ZEPSNM) ASYNC(1)
!$ACC EXIT DATA DELETE(FG) ASYNC(1)
...
!$ACC&          DELETE(D%NPROCL,D%NPTRLS,D%MSTABF,D%NASM0,D%OFFSETS_GEMM1,D%OFFSETS_GEMM2)&
!$ACC&          DELETE(D%OFFSETS_GEMM_MATRIX,D%LEGENDRE_MATRIX_STRIDES) ASYNC(1)
...
!$ACC WAIT(1)
  ...
  CALL CLEAN_TRGTOL_CACHE(KRESOL)
