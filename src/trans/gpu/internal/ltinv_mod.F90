...
CONTAINS
  ...
  SUBROUTINE LTINV(ALLOCATOR,HLTINV,KF_UV,KF_SCALARS,&
    ...
    USE TPM_DISTR,              ONLY: D
    USE LTINV_PACK_MOD,         ONLY: LTINV_PACK_SPEC_PAIR, LTINV_PACK_UV_PAIR, &
     &                                LTINV_PACK_SPEC_NSDER_PAIR
    ...
    CALL GSTATS(470,0)
    IFIRST = 0
    LFUSED_UV_SCALAR = KF_UV > 0 .AND. KF_SCALARS > 0 .AND. &
                    & PRESENT(PSPSCALAR) .AND. .NOT. LSCDERS
    IF (KF_UV > 0 .AND. LVORGP) THEN
      CALL GSTATS(476,0)
      CALL LTINV_PACK_SPEC_PAIR(ZINPA,ZINPA0,ZINPS,ZINPS0,PSPVOR,KF_UV,IFIRST,&
    ...
    IF (KF_UV > 0 .AND. LDIVGP) THEN
      CALL GSTATS(476,0)
      CALL LTINV_PACK_SPEC_PAIR(ZINPA,ZINPA0,ZINPS,ZINPS0,PSPDIV,KF_UV,IFIRST,&
                              & IIN_STRIDES0,IIN0_STRIDES0)
      CALL GSTATS(476,1)
      IFIRST = IFIRST + KF_UV
    ENDIF
    IF (KF_UV > 0) THEN
      IF (LFUSED_UV_SCALAR) THEN
        CALL GSTATS(478,0)
        CALL LTINV_PACK_UV_PAIR(ZINPA,ZINPA0,ZINPS,ZINPS0,PSPVOR,PSPDIV,PSPSCALAR,&
                              & KF_UV,KF_SCALARS,IFIRST,IFIRST+KF_UV,IFIRST+2*KF_UV,&
                              & ZEPSNM,IIN_STRIDES0,IIN0_STRIDES0)
        CALL GSTATS(478,1)
      ELSE
        CALL GSTATS(477,0)
        CALL LTINV_PACK_UV_PAIR(ZINPA,ZINPA0,ZINPS,ZINPS0,PSPVOR,PSPDIV,PSPVOR,&
                              & KF_UV,0,IFIRST,IFIRST+KF_UV,0,&
                              & ZEPSNM,IIN_STRIDES0,IIN0_STRIDES0)
        CALL GSTATS(477,1)
      ENDIF
    ...
    IF (KF_SCALARS > 0) THEN
      ...
      IF(PRESENT(PSPSCALAR)) THEN
        IF (LSCDERS) THEN
          CALL GSTATS(479,0)
          CALL LTINV_PACK_SPEC_NSDER_PAIR(ZINPA,ZINPA0,ZINPS,ZINPS0,PSPSCALAR,KF_SCALARS,&
                                        & ISCALAR_OFFSET,IDER_OFFSET,ZEPSNM,IIN_STRIDES0,IIN0_STRIDES0)
          CALL GSTATS(479,1)
        ELSEIF (.NOT. LFUSED_UV_SCALAR) THEN
          CALL GSTATS(476,0)
          CALL LTINV_PACK_SPEC_PAIR(ZINPA,ZINPA0,ZINPS,ZINPS0,PSPSCALAR,KF_SCALARS,&
                                  & ISCALAR_OFFSET,IIN_STRIDES0,IIN0_STRIDES0)
          CALL GSTATS(476,1)
        ENDIF
      ...
      ELSE
        IF(PRESENT(PSPSC2) .AND. NF_SC2 > 0) THEN
          IF (LSCDERS) THEN
            CALL GSTATS(479,0)
            CALL LTINV_PACK_SPEC_NSDER_PAIR(ZINPA,ZINPA0,ZINPS,ZINPS0,PSPSC2(:,:),NF_SC2,&
                                          & ISCALAR_OFFSET+ISCALAR_COUNT,IDER_OFFSET+ISCALAR_COUNT,&
                                          & ZEPSNM,IIN_STRIDES0,IIN0_STRIDES0)
            CALL GSTATS(479,1)
          ELSE
            CALL GSTATS(476,0)
            CALL LTINV_PACK_SPEC_PAIR(ZINPA,ZINPA0,ZINPS,ZINPS0,PSPSC2(:,:),NF_SC2,&
                                    & ISCALAR_OFFSET+ISCALAR_COUNT,IIN_STRIDES0,IIN0_STRIDES0)
            CALL GSTATS(476,1)
          ENDIF
        ...
        IF(PRESENT(PSPSC3A) .AND. NF_SC3A > 0) THEN
          DO J3=1,UBOUND(PSPSC3A,3)
            IF (LSCDERS) THEN
              CALL GSTATS(479,0)
              CALL LTINV_PACK_SPEC_NSDER_PAIR(ZINPA,ZINPA0,ZINPS,ZINPS0,PSPSC3A(:,:,J3),NF_SC3A,&
                                            & ISCALAR_OFFSET+ISCALAR_COUNT,IDER_OFFSET+ISCALAR_COUNT,&
                                            & ZEPSNM,IIN_STRIDES0,IIN0_STRIDES0)
              CALL GSTATS(479,1)
            ELSE
              CALL GSTATS(476,0)
              CALL LTINV_PACK_SPEC_PAIR(ZINPA,ZINPA0,ZINPS,ZINPS0,PSPSC3A(:,:,J3),NF_SC3A,&
                                      & ISCALAR_OFFSET+ISCALAR_COUNT,IIN_STRIDES0,IIN0_STRIDES0)
              CALL GSTATS(476,1)
            ENDIF
        ...
        IF(PRESENT(PSPSC3B) .AND. NF_SC3B > 0) THEN
          DO J3=1,UBOUND(PSPSC3B,3)
            IF (LSCDERS) THEN
              CALL GSTATS(479,0)
              CALL LTINV_PACK_SPEC_NSDER_PAIR(ZINPA,ZINPA0,ZINPS,ZINPS0,PSPSC3B(:,:,J3),NF_SC3B,&
                                            & ISCALAR_OFFSET+ISCALAR_COUNT,IDER_OFFSET+ISCALAR_COUNT,&
                                            & ZEPSNM,IIN_STRIDES0,IIN0_STRIDES0)
              CALL GSTATS(479,1)
            ELSE
              CALL GSTATS(476,0)
              CALL LTINV_PACK_SPEC_PAIR(ZINPA,ZINPA0,ZINPS,ZINPS0,PSPSC3B(:,:,J3),NF_SC3B,&
                                      & ISCALAR_OFFSET+ISCALAR_COUNT,IIN_STRIDES0,IIN0_STRIDES0)
              CALL GSTATS(476,1)
            ENDIF
