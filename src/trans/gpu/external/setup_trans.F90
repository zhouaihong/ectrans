...
#ifndef TRANS_SINGLE
USE BUTTERFLY_GPU_DP_MOD,        ONLY: BUILD_GPU_BUTTERFLY_PLANS
USE LEGENDRE_RECURRENCE_GPU_DP_MOD, ONLY: BUILD_LEGENDRE_RECURRENCE_DATA
#endif
#ifdef ACCGPU
...
IF( .NOT.LLSPSETUPONLY ) THEN
  ...
  CALL SUMP_TRANS
#ifndef TRANS_SINGLE
  IF (.NOT.D%LGRIDONLY) CALL BUILD_GPU_BUTTERFLY_PLANS
#endif
  CALL GSTATS(1802,0)
...
IF( .NOT.D%LGRIDONLY ) THEN
  ...
  CALL PREPSNM
#ifndef TRANS_SINGLE
  CALL BUILD_LEGENDRE_RECURRENCE_DATA
#endif
  ...
  !$ACC ENTER DATA COPYIN(FG%ZREC_A1,FG%ZREC_A2,FG%ZREC_A3,FG%ZREC_S1,FG%ZREC_S2,FG%ZREC_S3) &
  !$ACC& IF(ALLOCATED(FG%ZREC_A1)) ASYNC(1)
  !$ACC ENTER DATA COPYIN(FG%ZREC_MU2,FG%IREC_A_SEED,FG%IREC_S_SEED) &
  !$ACC& IF(ALLOCATED(FG%ZREC_MU2)) ASYNC(1)
  !$ACC ENTER DATA COPYIN(FG%IREC_TILE_MLOC,FG%IREC_TILE_ROW) &
  !$ACC& IF(ALLOCATED(FG%IREC_TILE_MLOC)) ASYNC(1)
#ifdef _CRAYFTN
  !$ACC ENTER DATA COPYIN(D,D%NUMP,D%MYMS,D%NPNTGTB0,D%NPNTGTB1,D%NSTAGT0B,D%NSTAGT1B,D%NSTAGTF,D%NPROCM,D%NPROCL)&
  !$ACC&           COPYIN(D%NPTRLS,D%MSTABF,D%NASM0,D%OFFSETS_GEMM1,D%OFFSETS_GEMM2,D%OFFSETS_GEMM_MATRIX) &
  !$ACC&           COPYIN(D%LEGENDRE_MATRIX_STRIDES,D%NDGL_FS) ASYNC(1)
#else
  !$ACC ENTER DATA COPYIN(D,D%MYMS,D%NPNTGTB0,D%NPNTGTB1,D%NSTAGT0B,D%NSTAGT1B,D%NSTAGTF,D%NPROCM,D%NPROCL)&
  !$ACC&           COPYIN(D%NPTRLS,D%MSTABF,D%NASM0,D%OFFSETS_GEMM1,D%OFFSETS_GEMM2,D%OFFSETS_GEMM_MATRIX) &
  !$ACC&           COPYIN(D%LEGENDRE_MATRIX_STRIDES) ASYNC(1)
#endif
  ...
  !$OMP TARGET ENTER DATA MAP(TO:FG%ZREC_A1,FG%ZREC_A2,FG%ZREC_A3,FG%ZREC_S1,FG%ZREC_S2,FG%ZREC_S3) &
  !$OMP& IF(ALLOCATED(FG%ZREC_A1))
  !$OMP TARGET ENTER DATA MAP(TO:FG%ZREC_MU2,FG%IREC_A_SEED,FG%IREC_S_SEED) &
  !$OMP& IF(ALLOCATED(FG%ZREC_MU2))
  !$OMP TARGET ENTER DATA MAP(TO:FG%IREC_TILE_MLOC,FG%IREC_TILE_ROW) &
  !$OMP& IF(ALLOCATED(FG%IREC_TILE_MLOC))
  ...
  !$OMP&                         D%OFFSETS_GEMM2,D%OFFSETS_GEMM_MATRIX,D%LEGENDRE_MATRIX_STRIDES,D%NDGL_FS)
