...
USE TPM_GEN,           ONLY: NERR, NOUT, NPRINTLEV, NPROMATR, LSYNC_TRANS
...
! 260420 wrqt begin
! 260420 wrqt comment输出GPU逆变换外层入口的关键布局量，便于核对输入输出场数与工作区尺寸
IF (NPRINTLEV > 0 .AND. MYPROC == 1 .AND. .NOT. LREPORTED) THEN
  WRITE(NOUT,'(A,8(I0,1X))') 'TRACE GPU INV_TRANS: IF_UV_G IF_SCALARS_G IF_UV IF_SCALARS IF_SCDERS IF_UV_PAR IF_GP IF_FS = ', &
    & IF_UV_G, IF_SCALARS_G, IF_UV, IF_SCALARS, IF_SCDERS, IF_UV_PAR, IF_GP, IF_FS
  WRITE(NOUT,'(A,3(I0,1X))') 'TRACE GPU INV_TRANS: IF_OUT_LT NPROMA NGPBLKS = ', IF_OUT_LT, NPROMA, NGPBLKS
  WRITE(NOUT,'(A,4(L1,1X))') 'TRACE GPU INV_TRANS: LVORGP LDIVGP LUVDER LSCDERS = ', &
