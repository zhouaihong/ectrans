...
USE TPM_GEN,           ONLY: NERR, NOUT, NPRINTLEV, LSYNC_TRANS
USE TPM_TRANS,         ONLY: LDIVGP, LSCDERS, LUVDER, LVORGP, LATLON, NF_SC2, NF_SC3A, NF_SC3B, &
  &                          NGPBLKS, NPROMA
USE TPM_DISTR,         ONLY: D, NPRTRV, MYPROC, MYSETV
USE TPM_FLT,           ONLY: S
...
! 260420 wrqt begin
! 260420 wrqt comment输出GPU直接变换外层入口的关键布局量，便于核对场数与打包尺寸
IF (NPRINTLEV > 0 .AND. MYPROC == 1 .AND. .NOT. LREPORTED) THEN
  WRITE(NOUT,'(A,6(I0,1X))') 'TRACE GPU DIR_TRANS: IF_UV_G IF_SCALARS_G IF_UV IF_SCALARS IF_GP IF_FS = ', &
    & IF_UV_G, IF_SCALARS_G, IF_UV, IF_SCALARS, IF_GP, IF_FS
  WRITE(NOUT,'(A,2(I0,1X))') 'TRACE GPU DIR_TRANS: NPROMA NGPBLKS = ', NPROMA, NGPBLKS
  WRITE(NOUT,'(A,5(L1,1X))') 'TRACE GPU DIR_TRANS: LATLON LSCDERS LVORGP LDIVGP LUVDER = ', &
    & LATLON, LSCDERS, LVORGP, LDIVGP, LUVDER
  LREPORTED = .TRUE.
ENDIF
! 260420 wrqt end
